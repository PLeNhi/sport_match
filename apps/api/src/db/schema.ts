...
export const hostProfiles = pgTable('HostProfile', {
  id: text('id').primaryKey().$defaultFn(() => crypto.randomUUID()),
...
export const venues = pgTable('Venue', {
  ...
  cityDistrictIdx: index('Venue_city_district_idx').on(table.city, table.district),
}));

// GameSession model - badminton session
export const gameSessions = pgTable('GameSession', {
  id: text('id').primaryKey().$defaultFn(() => crypto.randomUUID()),
  hostId: text('hostId').references(() => users.id, { onDelete: 'cascade' }).notNull(),
...
export const sessionParticipants = pgTable('SessionParticipant', {
  id: text('id').primaryKey().$defaultFn(() => crypto.randomUUID()),
  sessionId: text('sessionId').references(() => gameSessions.id, { onDelete: 'cascade' }).notNull(),
  ...
  sessionUserUnique: uniqueIndex('SessionParticipant_sessionId_userId_key').on(table.sessionId, table.userId),
}));
