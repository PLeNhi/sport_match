...
export class SessionsController {
  ...
  async findAll(
    ...
    const sessions = await this.sessionsService.findAll({
      ...
      district,
      date,
      skillLevel: skillLevel && Object.values(SKILL_LEVELS).includes(skillLevel as any) ? (skillLevel as any) : undefined,
      hostId,
  ...
  @Post(':id/join')
  async joinSession(@Param('id') id: string, @Request() req: AuthenticatedRequest) {
    const userId = await this.extractUserId(req);
    ...
    if (session.status !== 'open' && session.status !== 'full') {
      throw new BadRequestException(`Cannot join session with status ${session.status}`);
    }
    ...
    if (updated && (updated.joinedCount ?? 0) >= updated.maxPlayers && updated.status !== 'full') {
  ...
  @Post(':id/leave')
  async leaveSession(@Param('id') id: string, @Request() req: AuthenticatedRequest) {
    const userId = await this.extractUserId(req);

    const participant = await this.participantsService.findBySessionAndUser(id, userId);
    if (!participant) {
    ...
    const session = await this.sessionsService.findById(id);
    if (session && session.status === 'full' && (session.joinedCount ?? 0) < session.maxPlayers) {
