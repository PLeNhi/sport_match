...
export class ParticipantsController {
  ...
  async getSessionParticipants(@Param("sessionId") sessionId: string) {
    const participants =
      await this.participantsService.findBySession(sessionId);
    return { participants };
  }
  ...
  async removeParticipant(
    ...
    if (!session || session.hostId !== userId) {
      throw new BadRequestException(
        "Only session host can remove participants",
      );
    }
    ...
    const refreshed = await this.sessionsService.findById(
      participant.sessionId,
    );
    if (
      refreshed &&
      refreshed.status === "full" &&
      (refreshed.joinedCount ?? 0) < refreshed.maxPlayers
    ) {
